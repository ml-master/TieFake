...
folder = 'v3-4*'
df_train = pd.read_csv(f'./Data/{folder}/gossipcop_train.tsv',encoding='utf-8',delimiter="\t")
df_test = pd.read_csv(f'./Data/{folder}/gossipcop_test.tsv',encoding='utf-8',delimiter="\t")
# 数据加载【对于gossipcop_v3-1_style_based_fake.tsv是origin_text和generated_text】
# origin_text
# df_train = df_train[df_train['origin_text'].notna()]
# df_test = df_test[df_test['origin_text'].notna()]
# generated_text
...
bert_model = BertForSequenceClassification.from_pretrained("./bert-base-uncased", num_labels = num_of_way, output_attentions = False, output_hidden_states = False )
...
if skip_train:  
    bert_model = BertForSequenceClassification.from_pretrained(f'./Data/{folder}/bert_save_dir/')
else: 
    bert_model.save_pretrained(f'./Data/{folder}/bert_save_dir/')
    with open(f'./Data/{folder}/bert_training_stats.txt', 'w') as filehandle:
        json.dump(bert_training_stats, filehandle)
# ========================================
#               Testing
# ========================================
print('Number of test sentences: {:,}\n'.format(df_test.shape[0]))
sentences = df_test['title'].values
...
with open(f'./Data/{folder}/gossipcop_eval_report.json', 'w') as filehandle2:
