...
class ModelTrainer:
    ...
    def train_model(self, criterion, optimizer, scheduler, device, num_epochs=2, report_len=500):
        self.device = device
    ...
    def generate_eval_report(self, json_path):
        ...
        with torch.no_grad():
            for batch in tqdm(self._dataloaders['test']):
                ...
                # t_pred = bat_out > 0.5
                t_pred = (bat_out > 0.5).int()
                # l_pred.append(t_pred.squeeze().cpu().numpy())
                # l_labels.append(bat_labels.squeeze().cpu().numpy())
                l_pred.extend(t_pred.squeeze().cpu().numpy())
                l_labels.extend(bat_labels.squeeze().cpu().numpy())
        
        l_pred = np.array(l_pred)
        l_labels = np.array(l_labels)
        # 确保l_pred和l_labels是1D数组
        if l_pred.ndim > 1:
            l_pred = l_pred.flatten()
        if l_labels.ndim > 1:
            l_labels = l_labels.flatten()

        print("confusion_matrix(l_labels, l_pred)", confusion_matrix(l_labels, l_pred))
        print("f1_score(l_labels, l_pred): ",f1_score(l_labels, l_pred))
        print("matthews_corrcoef(l_labels, l_pred)", matthews_corrcoef(l_labels, l_pred))
        print("accuracy_score(l_labels, l_pred)", accuracy_score(l_labels, l_pred))
